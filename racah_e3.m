    ...
    elseif Li==Lj & Si==Sj
      if vi == vj
        ...
        elseif n==(v+2)                              % Eqn 82 and 84a of Racah IV
          e3(i,j) = ((1-v)/(7-v)) * ( racah_yfn(vi,vi,Si,Ui,vi,Uj) * racah_phi(Ui,Uj,Li) );
        elseif n==(v+4)                              % Eqn 82 and 84b of Racah IV
          e3(i,j) = (-4/(7-v))    * ( racah_yfn(vi,vi,Si,Ui,vi,Uj) * racah_phi(Ui,Uj,Li) );
        else                                         % Eqn 87 of Racah IV
          e3(i,j) = racah_yfn(n,v,Si,Ui,v,Uj) * racah_phi(Ui,Uj,Li);
	end
      elseif n==5 & vi==1 & vj==3 & Si==1/2          % Eqn 86a of Racah IV
        e3(i,j) = sqrt(2/5) * ( racah_yfn(3,1,1/2,Ui,3,Uj) * racah_phi(Ui,Uj,Li) );
      elseif n==6 & vi==0 & vj==4 & Si==0            % Eqn 86b of Racah IV
        e3(i,j) = sqrt(9/5) * ( racah_yfn(4,0,0,Ui,4,Uj)   * racah_phi(Ui,Uj,Li) );
      elseif n==6 & vi==2 & vj==4                    % Eqn 86c of Racah IV
        e3(i,j) = sqrt(1/6) * ( racah_yfn(4,2,Si,Ui,4,Uj)  * racah_phi(Ui,Uj,Li) );
      elseif n==7 & vi==1 & vj==5 & Si==1/2          % Eqn 86d of Racah IV
        e3(i,j) = sqrt(3/2) * ( racah_yfn(5,1,1/2,Ui,5,Uj) * racah_phi(Ui,Uj,Li) );
      else                                           % Eqn 87 of Racah IV
        e3(i,j) = racah_yfn(n,vi,Si,Ui,vj,Uj) * racah_phi(Ui,Uj,Li);
      end
      if i==j
        L = racah_lconv(Li);
