...
k   = K(1);             Uk   = racah_Umat(n,l,k,states);
kp  = K(2); if kp~=k;   Ukp  = racah_Umat(n,l,kp,states);  else Ukp = Uk; end
kpp = K(3); if kpp~=kp; Ukpp = racah_Umat(n,l,kpp,states); else Ukpp = Ukp; end
    ...
    for b = 1:num_states   % Loop over psi_bar                             _
      ...
      %if (L(i) < (kp+L(p))) | (L(i) > abs(kp-L(p))) % Ensures the triad (k'L'L) is non-zero
        Vkkk(i,j) = Vkkk(i,j) + ( sixj([k kp kpp; L(p) L(b) L(i)]) * Uk(i,b)*Ukpp(b,p)*Ukp(p,j) ) ...
	                      - ( sixj([k kp kpp; l l l]) * ( Uk2 + Ukp2 + Ukpp2 - delta ) );
      %end % if (k'L'L)
      end % for p
      %end % if (kLL)
    end  % for b
